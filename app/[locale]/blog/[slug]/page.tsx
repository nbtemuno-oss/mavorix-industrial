...
export default async function BlogDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const isVerticalMillingPolandCase = post.slug === "hjy-4gt-vertical-turret-milling-machine-sourcing-poland";
  const isPofFilmMiddleEastCase = post.slug === "pof-shrink-film-full-container-sourcing-middle-east";
    ...
    <>
      ...
      <Container className={isSourcingCase ? "py-14" : "grid gap-8 py-16 lg:grid-cols-[1fr_300px]"}>
        <article className={isSourcingCase ? "prose-industrial mx-auto min-w-0 max-w-[900px] rounded-lg border border-slate-200 bg-white p-6 shadow-sm md:p-10" : "prose-industrial min-w-0 rounded-lg border border-slate-200 bg-white p-6 shadow-sm md:p-10"}>
          ...
          {isVerticalMillingPolandCase ? <p className="case-featured-caption">The complete HJY-4GT and its loose accessories recorded together before export packing for the Poland order.</p> : null}
          {isPofFilmMiddleEastCase ? <p className="case-featured-caption">Carton-packed POF shrink film pallets secured with an inflatable dunnage bag before the Middle East container shipment.</p> : null}
        ...
        {!isSourcingCase ? (
          <aside className="h-fit rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 space-y-3 text-sm font-semibold text-slate-600">
              ...
              {isCustomScrewsGuide ? <Link href={`/${locale}/products/industrial-fasteners/custom-cold-headed-screws-and-fasteners/`} className="block hover:text-signal">Custom Screws and Fasteners</Link> : null}
      ...
      {isSourcingCase ? (
              ...
              <div className="mx-auto max-w-[900px] rounded-lg bg-navy p-7 text-white shadow-industrial md:p-10">
                ...
                <p className="mt-4 max-w-3xl leading-8 text-slate-300">
                  {isLedFloodLightCase
                    ...
                    : isVerticalMillingPolandCase
                    ? "Send the workpiece drawing, material, machining operations, tolerances, tooling preference, site power and destination. MAVORIX can help compare machine configurations, coordinate agreed checks and follow export packing."
                    : isPofFilmMiddleEastCase
                    ? "Send the film format, width, thickness, roll length, core size, application, shrink equipment, quantity, packing and destination. MAVORIX can help compare suppliers, coordinate samples and follow export packing."
        ...
        <CTASection
          title={isCustomCuttingToolsGuide ? "Need a Custom Cutting Tool for a Specific Machining Operation?" : isCustomScrewsGuide ? "Need a Custom Screw Made to a Drawing or Sample?" : undefined}
          text={isCustomCuttingToolsGuide ? "Send the tool drawing, sample photos, workpiece drawing, machining details and required quantity. MAVORIX can help coordinate supplier evaluation and quotation in China." : isCustomScrewsGuide ? "Send the drawing or standard, material, finish, quantity, application and inspection requirements. MAVORIX can help coordinate supplier review, samples and quotation in China." : undefined}
        />
      )}
