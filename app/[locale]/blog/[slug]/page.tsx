...
export default async function BlogDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const isSourcingCase = isRexrothCase || isCeramicMediaCase || isLedFloodLightCase || isHydraulicValveSourcingCase || isStrappingMachineCase || isCountersunkScrewCase || isAirCompressorCase;
      ...
      <Container className={isSourcingCase ? "py-14" : "grid gap-8 py-16 lg:grid-cols-[1fr_300px]"}>
        <article className={isSourcingCase ? "prose-industrial mx-auto min-w-0 max-w-[900px] rounded-lg border border-slate-200 bg-white p-6 shadow-sm md:p-10" : "prose-industrial min-w-0 rounded-lg border border-slate-200 bg-white p-6 shadow-sm md:p-10"}>
          ...
          <IndustrialImage
            src={image.src}
            alt={image.alt}
            className={isHydraulicValveSourcingCase ? "mx-auto mb-8 aspect-[9/16] w-full max-w-[680px]" : isStrappingMachineCase ? "mx-auto mb-8 aspect-[3/4] w-full max-w-[680px]" : isCountersunkScrewCase ? "mx-auto mb-8 aspect-[459/538] w-full max-w-[720px]" : isAirCompressorCase ? "mx-auto mb-8 aspect-[403/544] w-full max-w-[720px]" : isSourcingCase ? "mb-8 aspect-[16/9]" : "mb-8 min-h-[320px]"}
            sizes={isHydraulicValveSourcingCase || isStrappingMachineCase || isCountersunkScrewCase || isAirCompressorCase ? "(min-width: 768px) 680px, 100vw" : isSourcingCase ? "(min-width: 1024px) 840px, 100vw" : "(min-width: 1024px) 760px, 100vw"}
            fit={isSourcingCase ? "contain" : "cover"}
            priority={isAirCompressorCase}
          />
            ...
            <Container>
              <div className="mx-auto max-w-[900px] rounded-lg bg-navy p-7 text-white shadow-industrial md:p-10">
                <h2 className="text-2xl font-black leading-tight md:text-3xl">
                  {isLedFloodLightCase ? "Planning an Industrial LED Flood Light Order?" : isCeramicMediaCase ? "Need Ceramic Tumbling Media for a New or Existing Finishing Process?" : isStrappingMachineCase ? "Planning an Automatic Strapping Machine Order?" : isCountersunkScrewCase ? "Need Stainless Steel Screws or Mixed Fasteners from China?" : isAirCompressorCase ? "Planning a Complete Industrial Air Compressor System?" : "Have a Hydraulic Valve or Mixed Spare-Parts List to Source?"}
                </h2>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <Button href={`/${locale}/contact/`}>{isLedFloodLightCase ? "Send Your Lighting Requirement" : isCeramicMediaCase ? "Send Your Requirement" : isStrappingMachineCase ? "Send Package Details" : isCountersunkScrewCase ? "Send Your Screw List" : isAirCompressorCase ? "Send Your Air Requirement" : "Send Your Parts List"}</Button>
                  <Button href={site.whatsappUrl} variant="secondary">{isLedFloodLightCase ? "Discuss the Project" : isCeramicMediaCase ? "Discuss a Bulk Order" : isStrappingMachineCase ? "Discuss the Machine" : isCountersunkScrewCase ? "Discuss the Fasteners" : isAirCompressorCase ? "Discuss the System" : "Discuss Your Requirement"}</Button>
                </div>
              </div>
            </Container>
          </section>
